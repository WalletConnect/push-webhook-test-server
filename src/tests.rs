...
use crate::{post_client_id, get_client_id, DynamoClient};
...
impl DynamoClient for MockDynamoDbClient {
    async fn put_client_id(&self, _table: &str, client_id: &str) -> Result<(), Error> {
    ...
    async fn get_client_id(&self, _table: &str, client_id: &str) -> Result<(), Error> {
      println!("client_id {}", client_id);
      if client_id.eq("client_id-not-existing") {
        Err(Error::from("Boom"))
      } else {
...
async fn test_post_nominal() {
  ...
  let response = post_client_id(request, mock_ddb_client, "test_table_name".into()).await.expect("failed to handle request");
...
async fn test_get_nominal() {
  ...
  let mock_ddb_client = MockDynamoDbClient{};
  let response = get_client_id(request, mock_ddb_client, "test_table_name".into()).await.expect("failed to handle request");
